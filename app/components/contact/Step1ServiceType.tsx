...
export const SERVICE_OPTIONS: ServiceOption[] = [
  ...
  {
    id: "shop_service", // <-- Changed from "tire_purchase" to match Step 2 config
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        {options.map((option) => {
          return (
            <label
              key={option.id}
              className="group relative flex items-start gap-3.5 p-5 bg-stone-950/60 border border-stone-900/40 rounded-xl transition-all duration-200 cursor-pointer hover:border-red-700/60 hover:bg-stone-950/90 active:scale-[0.98] has-[:checked]:border-red-900 has-[:checked]:bg-stone-950/95 has-[:checked]:ring-1 has-[:checked]:ring-red-900"
            >
